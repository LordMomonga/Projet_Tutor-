      ...
      <div className={`h-16 fixed top-0 z-40 ${scrollClass} flex justify-between items-center px-3 md:px-6 w-screen`}>
        ...
        <div className="flex items-center gap-3">
          <div className="relative" onClick={() => { selectLang.current?.classList.toggle("hidden"); }}>
            ...
            <div ref={selectLang} className="hidden capitalize absolute top-full right-px mt-1 py-2 w-40 rounded-lg bg-[#f7f7f7] shadow-none ring-1 ring-slate-900/5 text-sm leading-6 font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:highlight-white/5 border-2 border-white dark:border-transparent">
              <span className={`cursor-pointer hover:text-primary/60 flex items-center gap-2 px-3 py-1 ${lang === "fr" ? "text-primary dark:text-primary" : ""}`} onClick={() => { setLang("fr"); i18n.changeLanguage("fr"); }}>
                <img src={fr} className="h-5" alt="fr" />
                {t("french")}
              </span>
              <span className={`cursor-pointer hover:text-primary/60 flex items-center gap-2 px-3 py-1 ${lang === "en" ? "text-primary dark:text-primary" : ""}`} onClick={() => { setLang("en"); i18n.changeLanguage("en"); }}>
                <img src={en} className="h-5 text-white" alt="en" />
                {t("english")}
              </span>
            </div>
          </div>

          <div className="sm:cursor-pointer right-4 z-[999] rounded-xl bg-[#f7f7f7] dark:bg-tertiary/80 p-3 hover:shadow-card border-[1px] border-white dark:border-transparent" onClick={switchTheme}>
